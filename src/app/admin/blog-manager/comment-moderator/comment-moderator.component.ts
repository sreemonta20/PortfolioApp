    ...
    <div class="admin-page-wrap" style="max-width:900px">
      <div class="page-head mb-4">
        ...
        <div class="d-flex gap-2">
          ...
          <button
            class="btn-crystal btn-sm"
            [class.btn-amber]="showPending()"
            (click)="togglePending()"
          >
      ...
      <div class="glass-card p-4" *ngIf="!loading(); else loadTpl">
        <div *ngIf="filtered().length === 0" class="empty-state">
          ...
          <p>
            No comments {{ showPending() ? "pending moderation" : "found" }}.
          </p>
        </div>

        <div
          *ngFor="let c of filtered(); trackBy: trackBy"
          class="comment-row glass-card p-3 mb-2"
        >
          <div class="d-flex align-items-start gap-3">
            ...
            <div class="flex-1">
              <div class="d-flex align-items-center gap-2 mb-1 flex-wrap">
                ...
                <span
                  *ngIf="c.authorEmail"
                  style="font-size:0.72rem;color:var(--text-sub)"
                  >{{ c.authorEmail }}</span
                >
                <span class="badge-teal" *ngIf="c.isApproved"
                  ><i class="bi bi-check-circle-fill"></i>Approved</span
                >
                <span class="badge-amber" *ngIf="!c.isApproved"
                  ><i class="bi bi-clock"></i>Pending</span
                >
                <span
                  style="font-size:0.7rem;color:var(--text-sub)"
                  class="ms-auto"
                  >{{ c.createdAt | date: "MMM d, y . h:mm a" }}</span
                >
              </div>
              <p
                style="font-size:0.83rem;color:var(--text-sub);line-height:1.6;margin:0"
              >
                {{ c.body }}
              </p>
            </div>
          ...
          <div class="d-flex gap-2 mt-3 justify-content-end">
            ...
            <button
              class="btn-crystal btn-danger-crystal btn-sm"
  ...
  approve(id: string) {
    this.admin.approveComment(id).subscribe(() => {
      this.comments.update((cs) =>
        cs.map((c) => (c.id === id ? { ...c, isApproved: true } : c)),
      );
    });
  }

  togglePending() {
    this.showPending.update((v) => !v);
  }
